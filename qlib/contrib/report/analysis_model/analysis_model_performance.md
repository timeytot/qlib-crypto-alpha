...
Execution Flow of ic_hist_figure Creation in SubplotsGraphhttps://github.com/microsoft/qlib (Qlib repository - reference for the code context)The following code creates a 1×2 subplot figure:Left: IC histogram + kernel density curve (using DistplotGraph)
...
Step-by-Step Execution FlowSubplotsGraph.init() is called
Receives and stores:df = _ic_df.dropna() (usually a DataFrame with one column "IC")
kind_map = {"kind": "HistogramGraph", "kwargs": {}}
subplots_kwargs = {rows:1, cols:2, print_grid:False, subplot_titles:[...]}
sub_graph_data = _sub_graph_data (pre-prepared list, typically two items)
layout = {yaxis2: ..., xaxis2: ...}

Internal attributes are initialized  self._df = _ic_df.dropna()
...
_init_sub_graph_data() is NOT executed
Since self._sub_graph_data is provided, the automatic column-based subplot generation is bypassed.
_init_figure() is called (core logic)python

self._figure = make_subplots(**self._subplots_kwargs)

→ Creates an empty 1×2 subplot grid with titles ["IC", "IC Normal Dist. Q-Q"] (or similar)Then loops over each item in self._sub_graph_data:python

for column_name, column_map in self._sub_graph_data:

Item 1 – Left subplot (col=1)
Typical content: ("IC", dict(row=1, col=1, name="", kind="DistplotGraph", graph_kwargs=dict(bin_size=...)))column_name is str → enters elif isinstance(column_name, str) branch
kind = "DistplotGraph" (explicitly overridden by sub_graph_data)
Creates a DistplotGraph instance:df = self._df.loc[:, ["IC"]]
graph_kwargs = {"bin_size": ...}

DistplotGraph._get_data() → generates usually 2 traces (histogram + KDE curve)

Item 2 – Right subplot (col=2)
Typical content: (_qqplot_fig, dict(row=1, col=2))column_name is a go.Figure → enters if isinstance(column_name, go.Figure) branch
Directly assigns _graph_obj = _qqplot_fig
_graph_data = _qqplot_fig.data → usually 2 traces (scatter points + reference line)

For each graph object:python
...
One-Sentence SummaryThe code creates a 1×2 subplot where the left side uses DistplotGraph (overriding the default HistogramGraph) to show IC distribution with density curve, and the right side directly reuses a pre-generated Q-Q figure — this is why you see a distribution plot with curve on the left instead of a plain histogram.
