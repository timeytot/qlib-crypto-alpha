...
https://github.com/microsoft/qlib/blob/main/qlib/backtest/executor.py#L432    start_idx, end_idx = get_start_end_idx(sub_cal, trade_decision)   
...
https://github.com/microsoft/qlib/blob/main/qlib/backtest/executor.py#L433    if not self._align_range_limit or start_idx <= sub_cal.get_trade_step() <= end_idx:
    if self._align_range_limit:  
        # Strict mode: MUST be inside the allowed range
